<script lang="ts">
    ...
    onMount(() => {
        searchClient = algoliasearch(
            'HFWIBHZTEY',
            'd0bc11e479eb503c177255cb13f6fa4b'
        )
...
<main class="mt-12 flex items-center flex-col">
    ...
    <input bind:value={query} on:keyup={search}  class="mt-6 w-3/5 px-4 py-3 border shadow-sm rounded-xl focus:border-blue outline-none" type="text" placeholder="Tapez le nom d'un navire, d'une personne, une date...">
    {#if hits.length == 0}
    <div class="mt-16 flex flex-col items-center">
        <a href="/contact" class="bg-lightblue hover:bg-blue transition text-md py-2 px-4 rounded-xl text-white">Je ne trouve pas la personne...</a>
    ...
    {/if}
    {#each hits as hit}
    <div class="mt-4 py-4 shadow-sm rounded-xl w-3/5 bg-white hover:cursor-pointer hover:bg-extralightgray  transition">
        <h2 class="px-6 ">{hit.ship_name} - Au large de {hit.nearest_town} en {hit.country}</h2>
        <h3 class="px-6 text-sm">{hit.date}</h3>
    </div>
    <div class="bg-white rounded-md my-1 py-4 px-4 shadow border border-extralightgray hover:cursor-pointer hover:bg-extralightgray  transition">
        <a href="/blog/{hit.link}"><h1>{@html hit._highlightResult.title.value}</h1></a>
    </div>
    {/each}
